...
def get_training_data_set():
    ...
    获得训练数据集
    """
    image_loader = ImageLoader('train-images.idx3-ubyte', 100)
    label_loader = LabelLoader('train-labels.idx1-ubyte', 100)
...
def get_test_data_set():
    ...
    获得测试数据集
    """
    image_loader = ImageLoader('t10k-images.idx3-ubyte', 50)
    label_loader = LabelLoader('t10k-labels.idx1-ubyte', 50)
    return image_loader.load(), label_loader.load()
...
def evaluate(network, test_data_set, test_labels):
    ...
    for i in range(total):
        ...
        predict = get_result(list(network.predict(test_data_set[i])))
        if label != predict:
            error += 1
